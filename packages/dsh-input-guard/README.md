...
| `cloud-access-key` | `AKIA`/`ASIA` + 16 位 | block |
| `llm-api-key` | `sk-` / `sk-ant-` / `gsk_` | block |
| `vcs-token` | `ghp_` 系列 / `github_pat_` / `glpat-` | block |
| `slack-token` | `xox[baprs]-` | block |
| `google-api-key` | `AIza` + 35 位 | block |
| `jwt` | `eyJ…` 三段 Base64URL | block |
| `bearer-token` | `Bearer <20+>` | block |
| `basic-auth-url` | `scheme://user:pass@host` | block |
| `generic-secret-assignment` | `password=` / `api_key:` 等赋值 | mask |
| `cn-id-card` | 18 位身份证（含校验位） | mask |
| `bank-card` | 16–19 位卡号（过 Luhn） | mask |
| `cn-mobile` | 手机号（容忍 `+86`/`0086` 与空格、短横分组） | mask |
| `cn-landline` | 固定电话（要求分隔符，校验区号与位数） | mask |
| `cn-service-number` | 400/800 服务号与常见客服短号 | mask |
| `email` | 邮箱 | mask |
| `private-ip` | `10.` / `172.16-31.` / `192.168.` | mask |
