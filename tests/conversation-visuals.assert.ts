...
assert.match(sidebar, /dcu-compact-nav[\s\S]*sidebar.newTask[\s\S]*sidebar.search[\s\S]*sidebar.experts[\s\S]*sidebar.skills[\s\S]*sidebar.plugins[\s\S]*sidebar.connectors[\s\S]*sidebar.assistant/, '窄轨必须保留主导航图标（定时任务入口改由页脚承担）')
assert.doesNotMatch(sidebar, /<MenuIcon><ScheduleIcon \/><\/MenuIcon>\{t\('sidebar\.schedule'\)\}/, '定时任务不得再作为主导航项重复出现')
assert.match(sidebar, /dcu-footer-actions[\s\S]*className="dcu-footer-link"[\s\S]*dcu-settings-seat/, '「定时任务」入口必须位于「设置」正上方')
assert.match(sidebar, /className="dcu-footer-link"[^>]*aria-label=\{t\('sidebar\.schedule'\)\}[^>]*onClick=\{openSchedule\}/, '「定时任务」入口必须复用侧栏内的定时视图，不得再跳转设置分区')
assert.match(sidebar, /const openSchedule = \(\): void => \{\s*if \(!showSchedule\) \{ selectExternalSection\(t\('sidebar\.schedule'\)\); return \}/, '未安装定时插件时「定时任务」必须退回设置分区')
assert.match(sidebar, /if \(visualCompact\) expandSidebar\(\)\s*setImTab\('schedule'\)|if \(visualCompact\) expandSidebar\(\)\n\s*setImTab\('schedule'\)/, '「定时任务」必须与「定时」页签同源，并在窄轨下先展开侧栏')
assert.match(sidebar, /\.dcu-compact \.dcu-footer-link\{[^}]*width:36px[^}]*\}/, '窄轨「定时任务」入口只能保留图标')
assert.match(sidebar, /\.dcu-compact \.dcu-footer-link-label\{display:none\}/, '窄轨必须隐藏「定时任务」文字，避免与图标错位')
